...
| Feature | Description |
|---------|-------------|
| **Container isolation** | PID/mount/UTS namespaces, cgroups v2 (memory, CPU), pivot_root into image |
| **Name-based management** | Containers identified by `--name`, not autogenerated IDs |
| **Detached mode** | `-d`/`--detach` flag for background containers |
| **Attached mode** | Foreground mode with Ctrl+C graceful shutdown |
| **Zombie-proof stop** | SIGTERM → wait → SIGKILL with `/proc/<pid>/status` + cgroup verification |
| **Logging** | JSON-per-line log files, `--tail` and `--follow` support |
| **Lifecycle** | `run` / `ps` / `stop` / `start` / `kill` / `logs` / `rm` |
...
### Run a container with Forge

```bash
# Run a Python HTTP server in a container (attached)
sudo forge run --image forge-test-python.tar --name web --memory 128m

# Detached mode
sudo forge run -d --image forge-test-python.tar --name web --memory 128m
...
```bash
forge run   --image <tar> --name <name> [--memory <mb>] [--cpus <n>]
            [--env KEY=VAL] [-d] [cmd...]
forge ps    [--all]
forge stop  <name> [--timeout 10]
forge start <name>
forge kill  <name>
forge logs  <name> [--tail N] [--follow]
forge rm    <name>
```

| Command | Description |
|---------|-------------|
| `run` | Create and start a container (attached by default; `-d` for detached) |
| `ps` | List running containers (`--all` for all) |
| `stop` | Send SIGTERM, wait, then SIGKILL |
| `start` | Start an existing stopped container |
| `kill` | Send SIGKILL immediately |
| `logs` | Show logs (`--tail N`, `--follow`) |
| `rm` | Remove a stopped container |

Requirements: root (for namespaces + cgroups), Linux kernel, x86_64.
...
- [x] **Stage 2** — Forge as a standalone CLI: `forge run --image foo.tar --memory 512m`, works locally without orchestration
